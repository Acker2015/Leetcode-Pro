...
public class M_014 {
     ...
     * 处理亿、万级别
     * 1. 处理亿，只存在一个亿标志
     * 2. 处理万，只存在一个万标志（注意如果万的单位前边没有千的存在，需要插入一位零值）
     * 3. 处理余下位数，如果上层标志位"亿" 或者 上层标志为"万"&本层没有"千"位，那么将会需要插入一位零值
     *
     * @param s
     * @param lastFlag
     * @return
    ...
    private long convert(String s, Character lastFlag) {
        ...
        if (idx >= 0) {
            ...
            if (left < 0 || right < 0) return -1;
            return left*symbolMap.get('亿') + right;
        }
        idx = s.indexOf('万');
        if (idx > 0) {
            ...
            if (left < 0 || right < 0) return -1;
            return left*symbolMap.get('万') + right;
        }
        int startIdx = 0;
